...
cntind = strfind(BRD.NAME,'Denmark');
...
filename = 'DENMARK_output_1538_160516'
...
out_file_name = ['DENMARK_output',timstr,'.dat'];
...
BGS = m_shaperead('DK_buildings_OSM')
RDS = m_shaperead('DK_highways_OSM')
...
for idns = 1:length(binslct_stt)
    ...
    while cntval <= 1000000 & iind <= length(bincand_stt{idns}) & not(ismember(binslct_stt{idns}(iind),coord_blacklist))
        ...
        figure(543)
        pcolorjw(X(590:670,780:980),Y(590:670,780:980), log(heatshare(590:670,780:980)))
        hold on
