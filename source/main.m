%%
% *BOLD TEXT* addpath(genpath('C:\Users\ashreeta\Documents\Martin\OSM\'))
...
BRD = m_shaperead('ne_50m_admin_0_countries_lakes')

cntind = strfind(BRD.name,'United States');
...
grump_0 = importdata('nads00ag.asc',' ',6);
grump = grump_0;
...
filename = 'UNITED_STATES_output_1918_160331h'
...
out_file_name = ['output',timstr,'.dat'];
...
for istate = 46:length(cr_states)
    BGS = m_shaperead([upper(strrep(cr_states{istate},' ','_')), '_buildings'])
    RDS = m_shaperead([upper(strrep(cr_states{istate},' ','_')), '_roads'])

    bnd_stt_index = strfind(bnd_stt.NAME,cr_states{istate});
    bnd_stt_index = find(~cellfun(@isempty,bnd_stt_index))
