...
%%
bnd_stt = m_shaperead('cb_2014_us_state_5m')

state_name = bnd_stt.NAME

for istate = 1:length(bnd_stt.NAME)
    if strcmp(bnd_stt.NAME{istate}, state_name{stind})
         USST_ind = istate;       
    end
end
...
cntind = strfind(BRD.NAME,'Poland');
cntind = find(~cellfun(@isempty,cntind));

%% CENSUS REGIONS
% Import census region boundaries
bnd_censreg = m_shaperead('cb_2014_us_region_500k');
bnd_censreg_data = bnd_censreg.ncst;
%Select census region
bnd_cr = [bnd_censreg.ncst{3}(:,1:2);[nan nan]]
...
filename = 'POLAND_output_1501_160517'
...
out_file_name = ['POLAND_output',timstr,'.dat'];
out_file_name = 'POLAND_output_1501_160517.dat'
% cr_states = { ...
% 'Arizona', ...
% 'Colorado', ...
% 'Idaho', ...
...
BGS = m_shaperead('POLAND_buildings')
RDS = m_shaperead('POLAND_highways')

% bgs_bdg = cat(1,strfind({BGS.building{:}},'yes'));
% is_bgs = find(~cellfun(@isempty,bgs_bdg));

is_bgs = ones(size(BGS.ncst))==1;


bnd_stt_arr = [[nan nan]; bnd_cr; [nan nan]];
...
binslct_stt=cell(length(heatbins),1);
for ibin = 1:length(heatbins)
    bincand_stt{ibin} = (find(heatdens_stt>heatbins(ibin)-dbin/2 & heatdens_stt<heatbins(ibin)+dbin/2  & heatdens > 0));
...
%for idns = length(binslct_stt):-1:1
for idns = 57:-1:1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         figure(543)
%         pcolorjw(X(450:610,920:1200),Y(450:610,920:1200), log(heatshare(450:610,920:1200)))
%         hold on
%         plot(X(indcand), Y(indcand),'.r')
%         plot(X(indcand), Y(indcand),'.r')
%         plot(X(binslct_stt{idns}(iind)), Y(binslct_stt{idns}(iind)),'og')
%         hold off
%         drawnow
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     

        [132 istate iind length(binslct_stt{idns}) cntval idns length(binslct_stt)]
        ...
        try
            [time_dur{idns}(cntval), min_length{idns}(cntval), con_length{idns}(cntval), cpcn_length{idns}(cntval), NSTRV{idns}(cntval), NBGSV{idns}(cntval), NSTR{idns}(cntval), NBGS{idns}(cntval)] = create_network(coord, RDS, BGS, is_bgs, deg2mety,deg2metx);
